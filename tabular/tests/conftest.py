...
def pytest_configure(config):
    ...
    config.addinivalue_line("markers", "platform: mark test as Ubuntu/Linux platform test")
...
def pytest_collection_modifyitems(config, items):
    ...
    skip_platform = pytest.mark.skip(reason="need --runplatfrom option to run")
    custom_markers = dict(slow=skip_slow, regression=skip_regression, pyodide=skip_pyodide, platform=skip_platform)
