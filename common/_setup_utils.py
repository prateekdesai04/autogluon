...
def _find_autogluon_root():
    """Find the autogluon root directory by looking for the VERSION file.

    This works in multiple scenarios:
    1. When _setup_utils.py is at repo root (development or PyPI source distribution)
    2. When _setup_utils.py is in core/src/autogluon/core/ (legacy location)
    """
    current_file = os.path.abspath(__file__)
    current_dir = os.path.dirname(current_file)

    # Scenario 1: _setup_utils.py at repo root
    candidate = current_dir
    if os.path.isfile(os.path.join(candidate, "VERSION")):
        return candidate

    # Scenario 2: _setup_utils.py in core/src/autogluon/core/ (normal repo structure)
    candidate = os.path.abspath(os.path.join(current_dir, "..", "..", "..", ".."))
    if os.path.isfile(os.path.join(candidate, "VERSION")):
        return candidate

    # Fallback: assume repo root is one level up from current directory
    return os.path.dirname(current_dir)


AUTOGLUON_ROOT_PATH = _find_autogluon_root()
